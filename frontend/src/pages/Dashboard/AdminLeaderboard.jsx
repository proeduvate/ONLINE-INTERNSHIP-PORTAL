...
export default function AdminLeaderboard({ usersList }) {
  ...
  React.useEffect(() => {
    ...
    const fetchLeaderboard = async () => {
      ...
      try {
        let periodParam = "all";
        if (timeFilter === 'Weekly') periodParam = 'weekly';
        else if (timeFilter === 'Monthly') periodParam = 'monthly';
        
        const response = await fetch(`http://localhost:8000/leaderboard?period=${periodParam}`);
        if (response.ok) {
          const data = await response.json();
          // API returns: rank, user_id, user_name, batch, domain, total_points
          // We map it to the structure the table expects
