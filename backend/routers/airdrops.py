...
@router.post("", response_model=schemas.AirdropResponse, status_code=status.HTTP_201_CREATED)
def create_airdrop(
    data: schemas.AirdropCreate,
    ...
    new_airdrop = models.BonusAirdrop(
        ...
        status=schemas.AirdropStatus.PENDING_APPROVAL.value
    )
    db.add(new_airdrop)
    db.commit()
...
@router.get("", response_model=List[schemas.AirdropResponse])
def get_airdrops(
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)
):
    if current_user.role.lower() in ["admin", "mentor"]:
        ...
        query = db.query(models.BonusAirdrop).filter(
            models.BonusAirdrop.status.in_([schemas.AirdropStatus.PUBLISHED.value, schemas.AirdropStatus.FINALIZED.value])
        )
        airdrops = query.all()
        
    # Lazy Auto-finalize evaluation
    for a in airdrops:
        if a.status == schemas.AirdropStatus.PUBLISHED.value:
...
@router.post("/{airdrop_id}/submit-approval", response_model=schemas.AirdropResponse)
def submit_for_approval(
    ...
    if airdrop.status != schemas.AirdropStatus.DRAFT.value:
        raise HTTPException(status_code=400, detail="Only DRAFT airdrops can be submitted for approval")
        
    airdrop.status = schemas.AirdropStatus.PENDING_APPROVAL.value
    db.commit()
    db.refresh(airdrop)
    return _build_airdrop_response(db, airdrop)

@router.post("/admin/{airdrop_id}/approve", response_model=schemas.AirdropResponse)
def approve_airdrop(
    airdrop_id: int,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)
    ...
    if airdrop.status != schemas.AirdropStatus.PENDING_APPROVAL.value:
        raise HTTPException(status_code=400, detail="Airdrop is not pending approval")
        
    airdrop.status = schemas.AirdropStatus.PUBLISHED.value
...
@router.post("/admin/{airdrop_id}/reject", response_model=schemas.AirdropResponse)
def reject_airdrop(
    airdrop_id: int,
    reason: str,
    db: Session = Depends(get_db),
    ...
    airdrop.status = schemas.AirdropStatus.DRAFT.value
    airdrop.rejection_reason = reason
    db.commit()
    db.refresh(airdrop)
    return _build_airdrop_response(db, airdrop)

@router.post("/admin/{airdrop_id}/publish", response_model=schemas.AirdropResponse)
def publish_airdrop(
    airdrop_id: int,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)
    ...
    if airdrop.status != schemas.AirdropStatus.APPROVED.value:
...
@router.patch("/{airdrop_id}", response_model=schemas.AirdropResponse)
def handle_airdrop_action(
    airdrop_id: int,
    data: schemas.AirdropPatchRequest,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)
):
    airdrop = db.query(models.BonusAirdrop).filter(models.BonusAirdrop.id == airdrop_id).first()
    ...
    if data.action == schemas.AirdropAction.START:
        if airdrop.status != schemas.AirdropStatus.PUBLISHED.value:
    ...
    elif data.action == schemas.AirdropAction.SUBMIT:
        if airdrop.status != schemas.AirdropStatus.PUBLISHED.value:
            raise HTTPException(status_code=400, detail="Challenge is not currently open")
        ...
        is_late = False
        if airdrop.start_mode == schemas.StartMode.FLEXIBLE.value:
...
def _finalize_airdrop(db: Session, airdrop: models.BonusAirdrop, now: datetime):
    ...
    for atm in attempts:
        if atm.completed_at and atm.started_at:
            if airdrop.start_mode == schemas.StartMode.FIXED.value and airdrop.start_time:
    ...
    airdrop.status = schemas.AirdropStatus.FINALIZED.value
...
def _build_airdrop_response(db: Session, airdrop: models.BonusAirdrop) -> schemas.AirdropResponse:
    ...
    resp = schemas.AirdropResponse(**resp_dict)
    
    attempt_responses = []
    for atm in attempts:
        atm_resp = schemas.AirdropAttemptResponse.model_validate(atm)
    ...
    result_responses = []
    for res in results:
        res_resp = schemas.AirdropResultResponse.model_validate(res)
