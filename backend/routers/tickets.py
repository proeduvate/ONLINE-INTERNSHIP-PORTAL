...
def _check_ticket_access(ticket: models.Ticket, current_user: models.User) -> None:
    ...
    if current_user.role == models.UserRole.INTERN:
        ...
        # Mentor can only access if assigned
        if ticket.assigned_to != current_user.id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Access Denied: This ticket is not assigned to you"
            )
...
def get_tickets(
    ...
    if current_user.role == models.UserRole.ADMIN:
        ...
        query = db.query(models.Ticket).filter(models.Ticket.assigned_to == current_user.id)
...
def update_ticket(
    ticket_id: int,
    data: schemas.TicketPatchRequest,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)
):
    ...
    if action == schemas.TicketAction.ASSIGN:
    ...
    elif action == schemas.TicketAction.MESSAGE:
        if not data.message:
    ...
    elif action == schemas.TicketAction.RESOLVE:
        if current_user.role not in [models.UserRole.MENTOR, models.UserRole.ADMIN]:
    ...
    elif action == schemas.TicketAction.CLOSE:
        if current_user.role != models.UserRole.ADMIN:
