...
@router.get("", response_model=Union[schemas.DomainFactResponse, schemas.FactCompletedResponse])
def get_domain_facts(
    ...
    if not unseen_facts:
        ...
        if not all_facts:
            return schemas.FactCompletedResponse(
    ...
    return schemas.DomainFactResponse(
        id=selected_fact.id,
