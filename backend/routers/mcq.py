...
from mcq_schemas import MCQStartResponse, MCQSubmitRequest, MCQResultResponse, MCQQuestionSchema
...
def load_day_questions_from_db(db: Session, domain_name: str, day: int):
    ...
    if not questions:
        questions = db.query(models.DomainMCQQuestion).filter(
            models.DomainMCQQuestion.domain_name.ilike(f"%{domain_name}%"),
            models.DomainMCQQuestion.day_number == day
        ).all()
        
    if not questions:
        domain = db.query(models.Domain).filter(models.Domain.name.ilike(f"%{domain_name}%")).first()
        if domain:
            task = db.query(models.Task).filter(
                models.Task.domain_id == domain.id,
                models.Task.day_number == day
            ).first()
            if task and task.mcq_questions:
                try:
                    parsed_qs = json.loads(task.mcq_questions)
                    formatted_questions = []
                    for idx, q in enumerate(parsed_qs):
                        opts = q.get("options", {})
                        if isinstance(opts, list):
                            opts_dict = {chr(65+i): str(o) for i, o in enumerate(opts)}
                        else:
                            opts_dict = opts
                        formatted_questions.append({
                            "id": f"TASK-MCQ-{task.id}-{idx+1}",
                            "question": q.get("question", q.get("text", "")),
                            "options": opts_dict,
                            "correct_answer": q.get("correct_option", q.get("correct_answer", "A"))
                        })
                    if formatted_questions:
                        return {"topic": task.title or "Daily MCQ Assessment", "questions": formatted_questions}
                except Exception:
                    pass
    
    if not questions:
        # Fallback to any available questions for that day if domain specific questions don't exist yet
        questions = db.query(models.DomainMCQQuestion).filter(
            models.DomainMCQQuestion.day_number == day
        ).all()
        
    if not questions:
        raise HTTPException(status_code=404, detail=f"Question bank for day {day} not found.")
...
def start_mcq_assessment(day: int, db: Session = Depends(database.get_db), current_user: models.User = Depends(get_current_user)):
    ...
    if not attempt:
        ...
        if not unique_bases and not all_questions:
            raise HTTPException(status_code=404, detail="No questions found in the question bank for this day.")
            
        num_to_select = min(10, len(unique_bases)) if unique_bases else min(10, len(all_questions))
        if unique_bases:
            selected_bases = random.sample(unique_bases, num_to_select)
